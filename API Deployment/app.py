...
# Route to perform OCR on a local image using NIB Document
@app.route('/ocr/local/nib', methods=['POST'])
def ocr_nib():
...
# Route to perform OCR on a local image using SIUP Document
@app.route('/ocr/local/siup', methods=['POST'])
def ocr_siup():
    # Get image path from request data
    image_path = request.form['image_path']
...
# Route to perform OCR on a local image using TDP Document
@app.route('/ocr/local/tdp', methods=['POST'])
def ocr_tdp():
    # Get image path from request data
    image_path = request.form['image_path']
...
# Route to perform OCR on a local image using SKDP Document
@app.route('/ocr/local/skdp', methods=['POST'])
def ocr_skdp():
    image_path = request.form['image_path']
...
# Route to perform OCR on a local image using NPWP Document
@app.route('/ocr/local/npwp', methods=['POST'])
def ocr_npwp():
    image_path = request.form['image_path']
...
# Route to perform OCR on a locak image using SIM Document
@app.route('/ocr/local/sim', methods=['POST'])
def ocr_sim():
    image_path = request.form['image_path']
    ...
    if allowed_file(image_path):
        custome_oef_config = r'--oem 3 --psm 6 outputbase digits'
        dict = {}
        pattern_on_sim_num = r'\d{4}-\d{4}-\d{6}'
        pattern_on_number = r"\d\."
        pattern_on_number_2 = r"\d\ "
        pattern_on_birth_data = r"^(.*), (\d{1,2}[-°]\d{2}[-/]\d{4})"
        pattern_date = r'(\d{1,2}-\d{2}-\d{4})'
        pattern_on_birth_data_3 = r'(\d{1,2}[-°]\d{2}-)(10|16)(\d{2})'
        ...
        # Set the maximum size for the resized image
        max_size = 670
        ...
        unsharp = cv2.addWeighted(resized_image, 2, gaussian, -0.5, 0)

        # Apply binary thresholding
        _, thresholded = cv2.threshold(unsharp, 120, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        

        cv2.imwrite('D:\Magang dll\Kampus Gratis (MSIB Batch 4)\coding\Inputting Automattion\Foto SIM\\thresh_gray SIM.jpg', thresholded)

        # perform text detection using Tesseract OCR
        text = pytesseract.image_to_string(thresholded, config = custome_oef_config)
        ...
        # pembuatan logika untuk menstore ktp number
        for text in list:
            try:
                if re.search(pattern_on_sim_num, text):
                    dict['ktp_number'] = re.search(pattern_on_sim_num, text).group()
                if 'ktp_number' not in dict:
                    dict['ktp_number'] = ''
            except:
                dict['ktp_number'] = ''
        
        # Pembuatan logika untuk menstore name
        for text in list:
            try:
                if re.search(pattern_on_sim_num, text):
                    index  = list.index(text) + 1
                    value = list[index].replace(':', '').replace('6b', 'b').replace('rata', 'ratna').replace('°','').strip()
                    if re.search(pattern_on_number, value):
        ...
        # Pembuatan logika untuk menstore tanggal dan tempat kelahiran
        for text in list:
            try:
                if re.search(pattern_on_birth_data, text):
                    value_place = re.search(pattern_on_birth_data, text).group(1).replace(',','').strip()
                    value_date = re.search(pattern_on_birth_data, text).group(2)
                    # change the 16y to 19y date of birth
                    value_date = re.sub(pattern_on_birth_data_3, r'\g<1>19\g<3>', value_date)
                    value_date = value_date.replace('°','-')
                    # Function to add leading zero to day if necessary
                    ...
                    # Replace dates in the text with the updated format
                    value_date = re.sub(pattern_date, add_leading_zero, value_date)

                    # exclude the number on place of birth value
                    if re.search(pattern_on_number_2, value_place):
                        value_place = value_place.split(re.search(pattern_on_number_2, value_place).group())[1].strip()
                    elif re.search(pattern_on_number, value_place):
                        value_place = value_place.split(re.search(pattern_on_number, value_place).group())[1].strip()

                    dict['place_of_birth'] = value_place
                    dict['date_of_birth'] = value_date
            except:
                dict ['place_of_birth'] = ''
                dict ['date_of_birth'] = ''
